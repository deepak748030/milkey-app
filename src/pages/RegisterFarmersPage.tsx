...
export function RegisterFarmersPage() {
    const [farmers, setFarmers] = useState<RegisterFarmer[]>([])
    ...
    const [editingFarmer, setEditingFarmer] = useState<RegisterFarmer | null>(null)
    ...
    const fetchFarmers = useCallback(async () => {
        try {
            setLoading(true)
            const res = await getRegisterFarmers({
    ...
    const openEditModal = (farmer: RegisterFarmer) => {
    ...
    const handleSubmit = async (e: React.FormEvent) => {
        e.preventDefault()
        if (!editingFarmer || !formData.name || !formData.mobile) return

        try {
            setSubmitting(true)
            await updateRegisterFarmer(editingFarmer._id, formData)
            setModalOpen(false)
            fetchFarmers()
        } catch (err) {
    ...
    const formatBalance = (balance: number): string => {
        ...
        return `₹${absBalance.toLocaleString('en-IN', { minimumFractionDigits: 2 })}`
    }

    const getBalanceColor = (balance: number): string => {
        if (balance > 0) return 'text-destructive' // Company owes farmer
        if (balance < 0) return 'text-success' // Farmer owes company
    ...
    return (
        <div className="space-y-6 animate-fade-in">
            ...
            <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4">
                <div>
                    <h1 className="text-2xl font-bold text-foreground">Register Farmers</h1>
                    <p className="text-muted-foreground">Manage registered farmers for milk purchase</p>
                </div>
                <div className="text-sm text-muted-foreground">
                    Total: <span className="font-medium text-foreground">{total}</span> farmers
                </div>
            </div>
                                    ...
                                    <tr key={farmer._id} className="hover:bg-muted/30 transition-colors">
                                        <td className="px-4 py-3">
                                            <div className="flex items-center gap-1.5">
                                                ...
                                                <span className="font-medium text-foreground">{farmer.code}</span>
                                            </div>
                                        </td>
                                        <td className="px-4 py-3">
                                        ...
                                        <td className="px-4 py-3">
                                            {farmer.address ? (
                                                <div className="flex items-center gap-1.5 text-muted-foreground">
                                                    ...
                                                    <span className="truncate max-w-[150px]">{farmer.address}</span>
                                                </div>
                                            ) : (
