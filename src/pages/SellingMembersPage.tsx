...
import { Search, Plus, Edit2, Trash2, Phone, MapPin, X, User } from 'lucide-react'
import {
    getSellingMembers,
    updateSellingMember,
    deleteSellingMember,
                                ...
                                {members.map(member => (
                                    <tr key={member._id} className="hover:bg-muted/30 transition-colors">
                                        ...
                                        <td className="px-4 py-3">
                                            <div className="flex items-center gap-1.5 text-muted-foreground">
                                                ...
                                                <span className="truncate max-w-[120px]">
                                                    {(member as any).owner?.name || 'Unknown'}
                                                </span>
                                            </div>
                                        </td>
                                        <td className="px-4 py-3">
                                        ...
                                        <td className="px-4 py-3 text-right text-muted-foreground">
                                            {member.totalLiters.toFixed(2)} L
                                        </td>
                                        <td className="px-4 py-3 text-right">
                                            <span className={cn(
                                                'font-medium',
                                                member.sellingPaymentBalance > 0 ? 'text-destructive' :
                                                    member.sellingPaymentBalance < 0 ? 'text-success' : 'text-foreground'
                                            )}>
                                                {formatCurrency(member.sellingPaymentBalance)}
                                            </span>
                                        </td>
                                        <td className="px-4 py-3">
